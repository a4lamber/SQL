/*
Problem: You want to sort the results of query by specific parts of a string.
For example, last two characters.

SUBSTR(string,start,length):
string (required): string to extract from.
start (required): the start position. If positive, extract from the beginning. If negative, extract from the end.
...
Question:
- 我记得每个row, 是定义50chars,如果是"hello"，则实际上在DB中，"hello      "是这样存的，
